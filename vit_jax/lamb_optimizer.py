...
class LAMB(OptimizerDef):
  ...
  def apply_param_gradient(self, step, hyper_params, param, state, grad):
    ...
    update = grad_ema / (jnp.sqrt(grad_sq_ema) + hyper_params.eps)
    if weight_decay != 0.0:
      update += weight_decay * param

    param_norm = jnp.linalg.norm(param)
    update_norm = jnp.linalg.norm(update)
    trust_ratio = jnp.where(param_norm > 0., jnp.where(
        update_norm > 0., param_norm / update_norm, 1.) ,1.)
    """
    from vit_jax import logging
    import os
    logdir = os.path.join("/tmp/vit_logs", "ViT-B-16")
    logger = logging.setup_logger(logdir)
    logger.info(trust_ratio)
    """
